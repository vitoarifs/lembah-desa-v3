<div>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-bold text-gray-800 leading-tight">Edit Menu: {{ $menu->nama }}</h2>
                <p class="text-xs text-gray-500 mt-1">Perbarui data detail hidangan</p>
            </div>
            ...
            <form wire:submit.prevent="update" class="space-y-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" wire:model="harga" class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-xs text-gray-900 focus:ring-2 focus:ring-amber-500">
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-1">Foto Menu (Kosongkan jika tidak diubah)</label>
                    ...
                    <div class="mt-2 flex items-center gap-4">
                        @if ($foto)
                            <div>
                                <span class="text-[10px] text-gray-400 block mb-1">Pratinjau Baru:</span>
                                <img src="{{ $foto->temporaryUrl() }}" class="w-20 h-20 object-cover rounded-lg border border-gray-300">
                            </div>
                        @elseif ($existingFoto)
                            <div>
                                <span class="text-[10px] text-gray-400 block mb-1">Foto Sekarang:</span>
                ...
                <div>
                    ...
                    <textarea wire:model="deskripsi" rows="3" class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg text-xs text-gray-900 focus:ring-2 focus:ring-amber-500"></textarea>
                    ...
                    <div class="space-y-2">
                        @foreach ($isi_paket as $index => $item)
                            <div class="flex items-center gap-2">
                                <input type="text" wire:model="isi_paket.{{ $index }}" class="w-full px-3 py-1.5 bg-white border border-gray-300 rounded-lg text-xs text-gray-900">
                ...
                <div class="pt-4 border-t border-gray-100 flex items-center justify-end gap-2">
                    ...
                    <button type="submit" class="px-4 py-2 bg-amber-600 hover:bg-amber-500 text-white text-xs font-semibold rounded-lg shadow-sm transition">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
